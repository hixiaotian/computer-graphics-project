...
{
	Chunk3DS	chunk;
	unsigned long		chunkStart = ftell(fp);
...
// 读入顶点
int C3dsReader::ReadPointArray (CTriObject*	newchild, unsigned long fileSize, FILE *fp) 
{
...
// 读入多边形
int C3dsReader::ReadFaceArray (CTriObject* newchild, unsigned long  fileSize, FILE* fp) 
{
...
// 读入对象所用的材质
int C3dsReader::ReadMeshMatGroup(CTriObject* newchild, MaterialDict* matdict, unsigned long fileSize, FILE* fp) 
{
...
// 读入对象数据
int C3dsReader::ReadTriObject(MaterialDict* matdict, 
							  ...
							  char* groupName)
{
	...
	Chunk3DS	chunk;
	unsigned long		chunkStart = ftell(fp);
	unsigned long		verticecount = 0;
	unsigned long			facecount = 0;
	unsigned long			matcount = 0;
	static long	id = 1;
...
int C3dsReader::ReadNamedObject(MaterialDict* matdict, unsigned long fileSize, unsigned long namedStart, unsigned long namedSize, FILE* fp)
{
	char		groupName[256];
	Chunk3DS	chunk;
...
// 读入材质定义，并将其添加到材质库中
int C3dsReader::ReadMatEntry(MaterialDict* matdict, unsigned long fileSize, unsigned long matStart, unsigned long matSize, FILE* fp)
{
	unsigned long			chunkStart = ftell(fp);
	Chunk3DS		chunk;
	char			name[256];
...
// 读入最高级的对象数据
int C3dsReader::ReadMDATA(MaterialDict* matdict, unsigned long fileSize, unsigned long mdataStart, unsigned long mdataSize, FILE* fp)
{
	unsigned long			chunkStart = ftell(fp);
	Chunk3DS		chunk;
...
// 读入3DS文件
int C3dsReader::Read3DSFile(unsigned long fileSize, unsigned long fileStart, unsigned long fileLen, FILE* fp)
{
	unsigned long			chunkStart = ftell(fp);
	Chunk3DS		chunk;
...
int C3dsReader::ReadKFDATA(unsigned long fileSize, unsigned long kfdataStart, unsigned long kfdataSize, FILE * fp)
{
	unsigned long		chunkStart = ftell(fp);
	Chunk3DS	chunk;
	short		version;
...
int C3dsReader::ReadKFObjectNode(unsigned long fileSize, unsigned long nodeStart, unsigned long nodeSize, FILE * fp)
{
...
int C3dsReader::ReadKFTrackTag(unsigned long fileSize, unsigned long tagStart, unsigned long tagSize, FILE * fp, char * nodeName, tVector* pivot, Chunk3DS	chunk)
{
	long  numkeys;
	short rflags;
	short trflags;
